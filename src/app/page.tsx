...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await getHotspotData(hotspotId.trim());
    ...
    if (result.error) {
        ...
        if (result.data.speciesAnalysis) {
          console.log('--- Server response for speciesAnalysis ---', result.data.speciesAnalysis);
        }
        setData(result.data);
    }
    setIsLoading(false);
      ...
      <main className="container mx-auto p-4 md:p-8">
        <header className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-2 max-w-2xl mx-auto">
            Get recent bird sightings from your favorite eBird hotspots. Just enter a hotspot ID to begin.
          </p>
        </header>
        ...
        <Card className="max-w-2xl mx-auto shadow-md border-2 border-primary/20">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2">
              ...
              />
              <Button type="submit" disabled={isLoading || !hotspotId} className="w-full sm:w-auto">
                {isLoading ? (
        ...
        <div className="max-w-4xl mx-auto mt-8">
          ...
          {data && (
            <SpeciesList data={data} softError={softError} />
          )}
        </div>
      </main>
