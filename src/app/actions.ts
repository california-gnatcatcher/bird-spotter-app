...
import type {
  EbirdHotspotInfo,
  EbirdChecklistSummary,
  SpeciesFrequencyAnalysisOutput,
} from "@/lib/types";
...
export async function getHotspotData(
  ...
  try {
    ...
    const safeMax = allowed.has(maxResults) ? maxResults : 20;

    const [infoResponse, checklistsResponse] = await Promise.all([
      fetch(`${EBIRD_API_BASE_URL}/ref/hotspot/info/${hotspotId}`, { headers }),
      fetch(
        `${EBIRD_API_BASE_URL}/product/lists/${hotspotId}?maxResults=${safeMax}&back=7`,
        { headers },
      ),
    ]);

    if (!infoResponse.ok || !checklistsResponse.ok)
      return { data: null, error: "Failed to fetch basic data." };

    const hotspotInfo = await infoResponse.json();
    const allChecklists: EbirdChecklistSummary[] =
      await checklistsResponse.json();
    const totalChecklists = allChecklists.length;

    if (totalChecklists === 0)
      return {
        data: { hotspotInfo, speciesAnalysis: [], totalChecklists: 0 },
        error: "No recent checklists.",
      };

    const regionCode =
      hotspotInfo.subnational2Code || hotspotInfo.subnational1Code;
    const notableResponse = await fetch(
      `${EBIRD_API_BASE_URL}/data/obs/${regionCode}/recent/notable?back=7&detail=full`,
      { headers },
    );
    ...
    const checklistPromises = allChecklists.map((checklist) =>
      fetch(
        `${EBIRD_API_BASE_URL}/product/checklist/view/${checklist.subId}?sppLocale=en`,
        { headers },
      )
        .then(async (res) => {
          if (!res.ok) return [];
          const d = await res.json();
          return (d.obs || []).map((o: any) => ({
            ...o,
    ...
    if (speciesCodes.length > 0) {
      try {
        const taxRes = await fetch(
          `${EBIRD_API_BASE_URL}/ref/taxonomy/ebird?fmt=json&species=${speciesCodes.join(",")}&sppLocale=en`,
          { headers },
        );
        if (taxRes.ok) {
          const taxData = await taxRes.json();
          taxData.forEach((item: any) => {
            if (speciesMap.has(item.speciesCode))
              speciesMap.get(item.speciesCode)!.commonName = item.comName;
          });
        }
      } catch (err) {
