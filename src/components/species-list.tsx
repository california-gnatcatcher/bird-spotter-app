...
export default function SpeciesList({ data, softError }: SpeciesListProps) {
  ...
  const selectedNames = useMemo(() => {
    ...
    return speciesAnalysis
      .filter(s => ids.has(getId(s)))
      .map(s => s.commonName);
  }, [selected, speciesAnalysis]);
  ...
  const selectAll = () => setSelected(new Set(speciesAnalysis.map(s => getId(s))));
  ...
  const copyNames = async () => {
    ...
    toast({
      title: "Copied!",
      description: `Copied ${selectedNames.length} species names to clipboard.`,
    });
  };
        ...
        <CardContent>
          ...
          {isSelectMode && (
            <div className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border p-3 bg-accent/20">
              <span className="text-sm font-semibold">Select species to copy names</span>
            ...
            <ul className="space-y-4">
              {speciesAnalysis.map((species: any, i: number) => {
                ...
                const frequencyPercent =
                  data.totalChecklists > 0 ? Math.round((checklistCount / data.totalChecklists) * 100) : 0;
                  ...
                  >
                        ...
                        <div className="space-y-1">
                          ...
                          <div
                            className={`flex items-center gap-1 text-[11px] ${
                              isRare ? 'text-orange-800/70' : 'text-muted-foreground'
                            }`}
                          >
                      ...
                      <div className="text-right">
                        ...
                        <div
                          className={`text-[10px] uppercase tracking-widest ${
                            isRare ? 'text-orange-800/60' : 'text-muted-foreground'
                          }`}
                        >
                          Total Seen
                        </div>
                    ...
                    <div className="flex items-center gap-3 mt-4">
                      ...
                      <p className={`text-sm whitespace-nowrap ${isRare ? 'text-orange-900/80' : 'text-muted-foreground'}`}>
                        <span className={`font-bold ${isRare ? 'text-orange-950' : 'text-foreground'}`}>{frequencyPercent}%</span>{' '}
